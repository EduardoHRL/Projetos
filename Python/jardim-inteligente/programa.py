...
def gerar_grafico_por_data(parent, data_selecionada, canvas_frame):
    ...
    dados_filtrados = [dado for dado in dados if datetime.strptime(dado['horario'], '%Y-%m-%dT%H:%M:%S').date() == data_filtrada]
...
def grafico(parent):
    ...
    btn_atualizar_grafico = ctk.CTkButton(parent, text="Gerar Gráfico", command=atualizar_grafico)
    btn_atualizar_grafico.pack(pady=10)
...
controle_manual_off_btn = ctk.CTkButton(tab_rele, text="Ligar irrigação Automática", command=desativar_controle_manual)
